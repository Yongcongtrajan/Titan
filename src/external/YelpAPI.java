...
public class YelpAPI{
	...
	private static final String API_KEY = "AYLUykjH4m8_S79FKc0ciiOyIpfa2peCXKGL_FLor08qJeMB_FVW7RJg4cLvlJK45nivfAUxp1w7iHPkz6LJy8H_LVHHp5yXQIzrKAsObJTgdhLUd5bMkMkcxqTnW3Yx";
...
return new ArrayList<>();
	...
	/*public JSONArray search(double lat, double lon, String term) {
		...
		// term=san%s20Jose&latitude=37078&longitude=-122.
		String url = HOST + ENDPOINT + "?" + query;
		...
		try {
			...
			BufferedReader in = BufferedReader(new InputStreamReader(connection.getInputStream()));
			...
			if(object.isNull("business")) {
				return object.getJSONArray("business);
			}
	...
	private void queryApi(double lat, double lon) {
		...
		try {
			...
			for(int i = 0; i < items.length(); ++i) {
				...
				System.out.println(itemtoString(2));
			}
		}
